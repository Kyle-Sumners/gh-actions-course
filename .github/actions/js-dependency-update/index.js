...
async function run() {
  ...
  if (!validateDirectoryName({ dirName: workingDir })) {
    core.error('Invalid directory name. Directory names should include only characters, numbers, hyphens, underscores, and forward slashes');
    return
  }
  
  logger.debug(`Base branch is ${baseBranch}`);
  logger.debug(`Target branch is ${headBranch}`);
  logger.debug(`Working directory i s${workingDir}`);
  ...
  if (gitStatus.stdout.length > 0) {
    ...
    try {
      ...
      logger.error('[js-dependency-update] : Something went wrong while creating the PR. Check logs below.');
